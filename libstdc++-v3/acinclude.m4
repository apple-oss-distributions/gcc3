...
AC_DEFUN(GLIBCPP_ENABLE_LONG_LONG, [dnl
  ...
  # APPLE LOCAL. Use strtoq, strtouq instead of strtoll, strtoull
dnl APPLE LOCAL. Use strtoq isntead of strtoll
  AC_TRY_COMPILE([#include <stdlib.h>],
                 [char* tmp; strtoq("gnu", &tmp, 10);],,[enable_long_long=no])
  AC_TRY_COMPILE([#include <stdlib.h>],
                 [char* tmp; strtouq("gnu", &tmp, 10);],,[enable_long_long=no])
