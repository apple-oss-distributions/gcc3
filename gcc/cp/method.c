     ...
     int emit_p;
{
  ...
  /* APPLE LOCAL begin thunk fix 20020125 turly  */
  /* A non-public comdat thunk can be made "public" if the thunkee is
     -- provided the thunkee is not inline.   */
  if (! SUPPORTS_ONE_ONLY && ! SUPPORTS_WEAK
      && DECL_LANG_SPECIFIC (thunk_fndecl) && DECL_COMDAT (thunk_fndecl)
      && ! TREE_PUBLIC (thunk_fndecl))
    {
      if (TREE_PUBLIC (function) && ! DECL_INLINE (function))
	TREE_PUBLIC (thunk_fndecl) = 1;
#ifdef COALESCE_STATIC_THUNK
      /* Coalesce static thunks to save space.  */
      else
	COALESCE_STATIC_THUNK (thunk_fndecl);
#endif
    }
  /* APPLE LOCAL end thunk fix 20020125 turly  */
