     ...
     int strict;
{
  ...
  switch (TREE_CODE (t1))
    ...
    case UNION_TYPE:
      ...
      if (compiling_objc && objc_comptypes (t1, t2, 0) == 1)
         return 1;
     ...
     int protect;
{
  ...
  if (DECL_FIELD_CONTEXT (field) != basetype)
      ...
      if (base != basetype && TYPE_BASE_CONVS_MAY_REQUIRE_CODE_P (basetype))
	{
	  ...
	  /* APPLE LOCAL permissive */
	  if (!flag_permissive && TREE_CODE (datum) == INDIRECT_REF
  ...
  if (TYPE_PTRMEMFUNC_P (TREE_TYPE (function)))
	...
	(PLUS_EXPR,
	 ...
	 vtbl, cp_convert (ptrdiff_type_node, delta2));
      ...
      if (flag_apple_kext)
	e2 = build_array_ref (vtbl,
		cp_build_binary_op (MINUS_EXPR, idx, integer_one_node));
      else
      /* APPLE LOCAL end 2.95-ptmf-compatibility  turly 20020314  */
     ...
     int require_complete, flags;
{
  ...
  if (TREE_CODE (function) == FUNCTION_DECL
    ...
    {
      /* If this is an overloaded builtin function, we have to select
         which overload to use now.  */
