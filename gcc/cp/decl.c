...
#if 0
      /* PB does not use namespace symbols.  But it complains about 
	 some namespace symbols.  Disable for now, until PB is ready
	 to accept namespace symbols. */
      if (flag_gen_index)
     ...
     int pseudo;
{
  ...
  s = (struct saved_scope *) PFE_CALLOC (1, sizeof (struct saved_scope), 
  				         PFE_ALLOC_SAVED_SCOPE);
  ...
  VARRAY_TREE_INIT (current_lang_base, 10, PFE_VARRAY "current_lang_base");
     ...
     tree decl;
{
  ...
  /* APPLE LOCAL PFE - PFE_VARRAY is pfe indicator or null */
  if (!local_names)
    VARRAY_TREE_INIT (local_names, 8, PFE_VARRAY "local_names");
     ...
     tree newdecl, olddecl;
{
  ...
  /* APPLE LOCAL Objective-C++ */
  /* Some ObjC++ metadata is fwd-declared as 'extern', only to be made 
     'static' later; we need to tolerate this, at least for now.  */
      || (compiling_objc && DECL_ARTIFICIAL (olddecl))
     ...
     const char *libname;
{
  ...
  /* All builtins that don't begin with an `_' should go in the `std'
     namespace.  */
  /* APPLE LOCAL  AltiVec  -- don't use `std' namespace for MD builtins.
                  We really need a better way of doing this!  */
  /* APPLE LOCAL alloca  -- donot' use `std' namespace for alloca.  */
  ...
  if (flag_gen_index)
    {
      ...
      switch (TREE_CODE (decl))
        {
           ...
           case TYPE_DECL:
             ...
             break;
           default:
             //fprintf ("indexing warning: Invalid decl TREE_CODE\n");
             break;
        }
      ...
      /* Generate inheritance info, for the class */
      if (DECL_CLASS_SCOPE_P (decl))
        {
     ...
     tree *attrlist;
{
     ...
     and get it as a string, for an error message.  */
  {
	...
	switch (TREE_CODE (decl))
	  ...
	  case IDENTIFIER_NODE:
		...
		/* APPLE LOCAL Objective-C++ */
		/* This should be finer-grained, but will do for now.  */
		if (!compiling_objc) 
  ...
  /* APPLE LOCAL begin AltiVec */
  /* pixel can only be used with vector.  */
  ...
  /* Check for the versions of vector.  */
  if (RIDBIT_SETP (RID_ALTIVEC_VECTOR, specbits))
    {
      /* Allow '__bool' as well as 'bool'.  */
     ...
     tree decl;
{
  ...
  /* Do not generate indexing info for compiler generated
     and template related stuff.  */
  if (processing_template_decl 
     ...
     tree decl;
{
  ...
  /* Make a copy.  */
  /* APPLE LOCAL PFE - Use PFE_MALLOC instead of xmalloc. */
  f = ((struct cp_language_function *)
       PFE_MALLOC (sizeof (struct cp_language_function),
       		   PFE_ALLOC_LANGUAGE_FUNCTION));
...
push_cp_function_context (f)
     struct function *f;
{
  /* APPLE LOCAL PFE - Use PFE_CALLOC instead of xmalloc. */
  struct cp_language_function *p
    = ((struct cp_language_function *)
       PFE_CALLOC (1, sizeof (struct cp_language_function),
       		   PFE_ALLOC_LANGUAGE_FUNCTION));
  ...
  while (p)
    {
      ...
      PFE_FREEZE_THAW_WALK (p->class_shadowed);
      PFE_FREEZE_THAW_WALK (p->type_shadowed);
      PFE_FREEZE_THAW_WALK (p->shadowed_labels);
      PFE_FREEZE_THAW_WALK (p->blocks);
      PFE_FREEZE_THAW_WALK (p->this_class);
...
/* Freeze/thaw struct named_label_use_list.  */
void 
pfe_freeze_thaw_named_label_use_list (pp)
     struct named_label_use_list **pp;
{
  struct named_label_use_list *p = (struct named_label_use_list *)PFE_FREEZE_THAW_PTR (pp);
...
/* Freeze/thaw struct named_label_list.  */
void 
...
/* Freeze/thaw static globals from this source - cp/decl.c.  */
void
  ...
  if (PFE_FREEZING)
    {
      ...
      hdr->only_namespace_names = only_namespace_names;
    }
  else
    {
      PFE_FREEZE_THAW_WALK (hdr->global_type_node);
      global_type_node = hdr->global_type_node;
...
/* We need get/set routines, because it is declared static.  */
int
pfe_get_anon_cnt ()
{
  return anon_cnt;
