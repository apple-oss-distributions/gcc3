...
struct lang_hooks
{
  ...
  /* Called to freeze/thaw language-dependent parts of a class 'd',
     class 't', and 'x'/'c' nodes.  See freeze-thaw.c for
     documentation.  */
  int (*pfe_freeze_thaw_decl)    PARAMS ((tree));
  int (*pfe_freeze_thaw_type)    PARAMS ((tree));
  int (*pfe_freeze_thaw_special) PARAMS ((tree));
