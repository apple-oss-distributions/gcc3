...
/*******************************************************************
 The following switch disables garbage collection for PFE.
 If defined to disable ggc_collect and also the freeze/thaw of the
 GC global data structures.  It needs to be disabled until we have
 an underlying memory managment freezing capability that can take
 a page-aligned pointer and retain it's page-aligned-ness when it
 is frozen and subsequently thawed.  Even if that done we still
 cannot use the GC since the LOOKUP_L1 and LOOKUP_L2 macros defined
 later generate page table indices which are a function of a page
 address and the thawed addresses of the pages are not the same as
 the addresses they originally had when frozen which produced the
 page tables.
 *****************************************************************/
...
/* APPLE LOCAL PFE */
#ifdef PFE
/* For PFE we will use mmap (if available) for loads but page groups
   for dump.  We will also need to to do a runtime test in some
   places to see if USING_MMAP is defined in the context of using
   page groups.  To that end PFE_USING_MMAP always has a 0 or 1
   value shadowing USING_MMAP undefined or defined status
   respectively.  */
#define USING_MALLOC_PAGE_GROUPS
...
/* Called by pfe_freeze_thaw_compiler_state() to freeze/thaw all the
   GC global data.  A copy of G is frozen/thawed and a pointer to it
   saved in pfe_compiler_state.ggc_globals.  The ptr to that ggc_globals
   field is passed.  For dumping, init_ggc() reserved the space and
   set ggc_globals.  We then copy and freeze G into that space.  For
   loading we thaw the pointer pointed to by ggc_globals and copy
   the entire thing to G.  */
void
...
/* Freeze/thaw a page_entry.  */
static void
freeze_thaw_page_entry (p)
     page_entry *p;
{ 
  /* Exit as soon as we detect whether we've previously seen this entry.  */
  while (p)
    {
     ...
     page_group **groupp;
{
  ...
  /* Exit as soon as we detect whether we've previously seen this group.  */
  while (group)
    {
...
/* Freeze/thaw page_tables when HOST_BITS_PER_PTR > 32.  */
static void
