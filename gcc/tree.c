...
/* APPLE LOCAL PFE */
#ifdef PFE
  /* We need alignment to avoid odd pointers.  */
  obstack_alignment_mask (&permanent_obstack) = 1;
#endif
     ...
     enum tree_code code;
{
  ...
  /* APPLE LOCAL PFE - expand to pfe_ggc_alloc_tree or ggc_alloc_tree */
...
#ifdef PFE
      if (!input_filename)
        input_filename = PFE_SAVESTRING ((void *)"<built-in>");
      else /*if (!pfe_is_pfe_mem ((void *)input_filename))*/
        input_filename = PFE_SAVESTRING ((void *) input_filename);
#endif
      DECL_SOURCE_FILE (t) =
