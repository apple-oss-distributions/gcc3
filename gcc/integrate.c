  ...
  if (ivs == 0)
    {
      ...
      fun->hard_reg_initial_vals 
        = (void *) PFE_MALLOC (sizeof (initial_value_struct),
        		       PFE_ALLOC_INITIAL_VALUE_STRUCT);
      ivs = fun->hard_reg_initial_vals;
      ivs->num_entries = 0;
      ivs->max_entries = 5;
      /* APPLE LOCAL PFE - expand to pfe_malloc or xmalloc  */
      ivs->entries 
        = (initial_value_pair *) PFE_MALLOC (5 * sizeof (initial_value_pair),
        				     PFE_ALLOC_INITIAL_VALUE_PAIR);
    }
  ...
  if (ivs->num_entries >= ivs->max_entries)
    {
      ...
      ivs->entries = (initial_value_pair *) PFE_REALLOC (ivs->entries,
							 ivs->max_entries
...
/* Called by pfe_freeze_thaw_function() to freeze/thaw the
   initial_value_struct (private to this file) field of a struct function.  */ 
void 
pfe_freeze_thaw_initial_value_struct (ivsp)
  struct initial_value_struct **ivsp;
{
  struct initial_value_struct *ivs = PFE_FREEZE_THAW_PTR (ivsp);
  struct initial_value_pair *entries;
  int i;
  
  if (!ivs)
    return;
    
  entries = PFE_FREEZE_THAW_PTR (&ivs->entries);
  if (!entries)
    return;
  
  for (i = 0; i < ivs->num_entries; i ++)
    {
      PFE_FREEZE_THAW_RTX (entries[i].hard_reg);
      PFE_FREEZE_THAW_RTX (entries[i].pseudo);
    }
}

/*-------------------------------------------------------------------*/

DEFINE_CHECK_STRUCT_FUNCTION (initial_value_struct)
DEFINE_CHECK_STRUCT_FUNCTION (initial_value_pair)
