...
/* A varasm_status contains pool_constant's and pool_constant's
   contain a pointer to a constant_descriptor (both defined in this
   file).  Both the pool_constant's and constant_descriptor's are
   part of their respective hash tables which are also part of a
   varasm_status.
   
   The hash functions for these tables are a function of an address
   for pool_constant's and the possiblity of an address embedded in
   an rtx in a constant_descriptor.  The addresses used during a PFE
   dump are not the same as those done for a PFE load.  Thus the hash
   values for a loaded set of pool_constant's and constant_descriptor's
   would not be correct using such hash functions.
   
   We could rebuild the hash tables based on their new load addresses
   using the normal hash functions when not using PFE.  But we could
   only do this if we are walking the various trees and hit the decl's
   for the functions.  Unfortunately (or is it fortunately?) when
   using PFE_NO_THAW_LOAD no thawing is necessary so we don't walk the
   various data structures.  Thus we wouldn't get the opportunity to
   rebuild the hash tables unless we make special provisions to do so.
   It's not worth it.  So the alternative is to use hash functions
   which are address independent.
   
   With PFE we replace the SYMHASH() macro with our own macro that
   does a call to symhash() to hash pool_constant's based on the
   string pointed to by the label rather than basing on the string
   address itself.  We also define a different const_hash_rtx()
   for constant_descriptor's which bases its hash value on just the
   non-address info in the rtx.  */ 
...
#if 0 /* APPLE FIXME we don't need this anymore, right? */
void
assemble_real (d, mode, align)
     REAL_VALUE_TYPE d;
     enum machine_mode mode;
...
#endif
  /* APPLE LOCAL POWER double fields
     Contrary to the FSF comment above, the most restrictive POWER
     structure alignment is 32-bits, UNLESS 1) the first field
     requires more alignment (e.g. first field is double), OR 2) any
     field is type vector (forces 16-byte alignment for the entire
     struct).  Until we know better, we'll assume the FE gets this
     correct.  GCC 2.95 has no similar check.  Disabling alignment
     check with "0 &&".  */
  ...
  if (desc == 0)
    {
	 ...
	 future calls to this function to find.  */
      ...
      /* APPLE LOCAL PFE - expand to pfe_ggc_strdup or ggc_strdup */
     ...
     rtx x;
{
  ...
  /* APPLE LOCAL PFE -- the previous call results in PFE_SAVESTRING
     being called on 'found', which is not what we want.  Point the
     SYMBOL_REF back to the garbage collector's version of 'found'.  */
#ifdef PFE
  if (PFE_FREEZING)
     ...
     struct varasm_status *v;
{
  ...
  /* There are two links on a pool_constant: the 'next_sym' which
     ...
     separately because the call to freeze_thaw_constant_descriptor()
...
/* Freeze/thaw a constant_descriptor pointed to from a pool_constant
   entry.  We only handle the one constant_descriptor in the
   pool_constant.  Since each constant_descriptor is created at the
   ...
   handle the corresponding constant_descriptor's u.contents field.  */
static void 
     ...
     rtx x;
{
  ...
  /* record_constant_rtx() is the one that allocates constant_descriptors
     (called by force_const_mem()).  record_constant_rtx() saves it's
     information in decl->u.contents of a constant_descriptor as a struct
     rtx_const * with decode_rtx_const().  So...  */
...
/* Calculate the hash of the label string.
   This code is basically a rip-off of calc_hash() in hashtable.c.
   Unlike the standard SYMHASH macro, which uses the label value
   (an address) itself as the basis for a hash value, here we use
   the string the label points to as the basis for a hash value
   thus making it address independent.  The SYMHASH macro is
   defined to call this function.  It passes 0 for add_to_total.
   But explicit calls to symhash() can pass a value to be added
   to the computation.  */
static int 
...
/* Compute a hash code for a constant RTL expression.
   This replaces the other const_hash_rtx() which bases it's hash value
   on the entire contents of a struct rtx_const value which may contain
   an address.  Here we use the address-independent parts of a struct
   rtx_const value to generate a hash value.  Its not as "good" as
   the other const_hash_rtx() for LABEL_REF, CONST, and PC rts'x
   (and possibly even SYMBOL_REF).  I'm open to suggestions :-)  */
static int
const_hash_rtx (mode, x)
     enum machine_mode mode;
     rtx x;
{
  ...
  switch (GET_CODE (x))
    ...
    case CONST:
      ...
      /* FIXME: what else can we do with value.un.addr.base here? */
      break;
