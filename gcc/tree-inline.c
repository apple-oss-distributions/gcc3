     ...
     void *data;
{
  ...
  if (TREE_ASM_WRITTEN (fn) || DECL_SAVED_INSNS (fn))
    {
      /* At this point the FUNCTION_DECL has had its parameters rearranged
	 for Altivec; the call has not.  Compensate.  Do not change
	 TREE_OPERAND (t, 1) here.  Save and restore saved_return_type 
	 analogously to calls.c:expand_call(). */
