...
static int substitute_pic_base_reg = -1;
     ...
     tree t2;
{
  ...
  if (!BUILTIN_volatile_ptr_ok(self)
      && TYPE_VOLATILE (t2))
    return 0;
  return comptypes (t1, lang_build_type_variant (t2, 0, 0), 0);
}
  ...
  if (GET_CODE (operands[0]) == REG && INT_REGNO_P (REGNO (operands[0])))
      ...
      switch (easy_vector_constant (operands[1]))
	{
	  ...
	  case 3: /* vone */
	    ...
	    break;
	  case 4: /* vspltisw */
	    immed = CONST_VECTOR_0 (operands[1]);
	    ...
	    break;
	  case 5: /* vspltish */
	    immed = CONST_VECTOR_0 (operands[1]) & 0xffff;
	    immed = (immed & 0x1f) | ((-((immed>>4)&1))<<5);
	    emit_insn (gen_xfA_perm (operands[0],
				     gen_rtx (CONST_INT, VOIDmode, immed),
				     gen_rtx (SYMBOL_REF, Pmode,
	    ...
	    break;
	  case 6: /* vspltisb */
	    immed = CONST_VECTOR_0 (operands[1]) & 0xff;
	    immed = (immed & 0x1f) | ((-((immed>>4)&1))<<5);
	    emit_insn (gen_xfA_perm (operands[0],
				     gen_rtx (CONST_INT, VOIDmode, immed),
				     gen_rtx (SYMBOL_REF, Pmode,
	  ...
	  case 7: /* lvsr 0,0 */
	    ...
	    break;
	  case 8: /* lvsl 0,immed */
	    immed = CONST_VECTOR_0 (operands[1]) >> 24;
	    emit_insn (gen_xfii_load (operands[0],
				      gen_rtx (CONST_INT, VOIDmode, (HOST_WIDE_INT)0),
				      copy_to_mode_reg (SImode,
							gen_rtx (CONST_INT,
     ...
     int flag_altivec;
{
  ...
  for (o = Overload; OVERLOAD_name (o); o++)
	...
	decl = builtin_function (OVERLOAD_name (o), void_ftype_any,
				 OVERLOAD_fcode (o), BUILT_IN_MD, NULL);
     ...
     int named;
{
  ...
  if (TARGET_ALTIVEC_ABI && ALTIVEC_VECTOR_MODE (mode))
    ...
    {
      /* APPLE LOCAL AltiVec */
      /* do we need to update our version from this? sts 2001-10-22 */
...
/* APPLE LOCAL this function and all calls to it */
     ...
     rtx operands[];
{
  ...
  if (TARGET_STRING)	/* string instructions are available */
    {
      for ( offset=0; bytes > 0; (bytes -= move_bytes), (offset += move_bytes))
	{
		   ...
		   && (align >= 8 || (! STRICT_ALIGNMENT && align >= 4)))
	    {
	      ...
	      emit_move_insn (tmp_reg,
			      expand_block_move_mem (DImode,
						     src_reg, orig_src, offset));
	      emit_move_insn (expand_block_move_mem (DImode,
						     dest_reg, orig_dest, offset),
			      tmp_reg);
	    }
     ...
     struct function *p;
{
  ...
  p->machine = (machine_function *) PFE_CALLOC (1, sizeof (machine_function),
		PFE_ALLOC_MACHINE_STATUS);
}
...
/* APPLE LOCAL PFE */
#ifdef PFE
/* Called by pfe_freeze_thaw_function() to freeze/thaw the 
   machine_function (private to this file) field of a struct function.  */ 
void 
rs6000_pfe_freeze_thaw_machine_status (p)
     struct function *p;
{
  PFE_FREEZE_THAW_PTR (&p->machine);
}
#endif /* PFE */
...
/* Calculate the stack information for the current function.  This is
   ...
   APPLE LOCAL: AltiVec
   or 16 bytes).  For Darwin/Mac OS X, it is 16 bytes.
...
rs6000_stack_info ()
{
  ...
  if (((TARGET_TOC && TARGET_MINIMAL_TOC)
       ...
       || (flag_pic && abi == ABI_DARWIN && substitute_pic_base_reg == -1))
...
/* Say whether a function is a candidate for sibcall handling or not.
   We do not allow sibling calls if -mlong_branch, nor
   do we allow indirect calls to be optimized into sibling calls.
   Also, we can't do it if there are any vector parameters; there's
   nowhere to put the VRsave code so it works.  (The tail recursion
   case can work, but there's no way to distinguish here.) */
int
function_ok_for_sibcall (fndecl)
    tree fndecl;
{
  tree type;
  if ( fndecl && ! TARGET_LONG_BRANCH )
    {
...
rs6000_emit_prologue ()
{
...
#if TARGET_MACHO
  /* APPLE LOCAL special ObjC method R12 use */
  if (DEFAULT_ABI == ABI_DARWIN 
  ...
  if (info->world_save_p)
	...
	{
	  rtx reg = gen_rtx_REG (V16QImode, info->first_altivec_reg_save + i);
	  rtx addr = gen_rtx_PLUS (Pmode, frame_reg_rtx,
      ...
      if (current_function_uses_pic_offset_table && flag_pic)
	{
	  ...
	  LABEL_ALTERNATE_NAME (picbase_label)
	    = GGC_STRDUP (machopic_function_base_name ());
	  insn = emit_label (picbase_label);
	  /* Make sure the scheduler doesn't insert any instructions
	     between the call above and this label.  Otherwise, the value
	     in the PIC base register will not point to this label!  */
  ...
  /* If we need to save CR, put it into r12.  
     For Darwin, use R11, so we don't clobber the special ObjC method use of R12.  */
  if (info->cr_save_p && frame_reg_rtx != frame_ptr_rtx)
    {
      cr_save_rtx = gen_rtx_REG (SImode, DEFAULT_ABI == ABI_DARWIN ? 11 : 12);
  ...
  if (objc_method_using_pic)
      rs6000_maybe_dead (emit_move_insn (gen_rtx_REG (Pmode,
						substitute_pic_base_reg == -1 
						? PIC_OFFSET_TABLE_REGNUM 
						: substitute_pic_base_reg),
					 gen_rtx_REG (Pmode, 12)));
	...
	if (regs_ever_live[i] && ! call_used_regs[i])
	  {
	    ...
	    /* APPLE LOCAL AltiVec V4SI -> V16QI */
	    savereg = gen_rtx_REG (V16QImode, i);
	    ...
	    /* APPLE LOCAL AltiVec V4SI -> V16QI */
	    mem = gen_rtx_MEM (V16QImode, addr);
	    set_mem_alias_set (mem, rs6000_sr_alias_set);
  ...
  if (info->vrsave_save_p)
      ...
      if (mask)
	{
	  ...
	  for (r = FIRST_ALTIVEC_REGNO; r <= LAST_ALTIVEC_REGNO; ++r)
	    if ((mask & ALTIVEC_REG_BIT (r)) != 0)
	      RTVEC_ELT (p, count++) =
		gen_rtx_CLOBBER (VOIDmode, gen_rtx_REG (V16QImode, r));
  ...
  if (! HAVE_prologue)
    {
	 ...
	 the "toplevel" insn chain.  */
      ...
      /* APPLE LOCAL support -mno-sched-prolog. */
	...
	if (regs_ever_live[i] && ! call_used_regs[i])
	  {
	    ...
	    /* APPLE LOCAL AltiVec V4SI -> V16QI */
	    emit_move_insn (gen_rtx_REG (V16QImode, i), mem);
	  }
    }
...
output_compiler_stub ()
{
  ...
  if (!flag_pic)
	...
	if (IDENTIFIER_POINTER (STUB_FUNCTION_NAME (stub))[0] == '*')
	  ...
	  {
	    char *munged, *stripped;

	    label_buf[0] = '_';
	    munged = IDENTIFIER_POINTER (STUB_FUNCTION_NAME (stub));
	    STRIP_NAME_ENCODING (stripped, munged);
	    strcpy (label_buf + 1, stripped);
	  }
     ...
     const char *symb, *stub;
{
  ...
  if (flag_pic == 2)
    ...
    /* APPLE LOCAL  dynamic-no-pic  */
    {
      ...
      fprintf (file, "\tlwz r12,lo16(%s)(r11)\n", lazy_ptr_name);
      fprintf (file, "\tmtctr r12\n");
      fprintf (file, "\taddi r11,r11,lo16(%s)\n", lazy_ptr_name);
      fprintf (file, "\tbctr\n");
    }
