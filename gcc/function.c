...
#include <assert.h>
struct temp_slot;
static void freeze_thaw_expr_status    PARAMS ((struct expr_status **));
static void freeze_thaw_emit_status    PARAMS ((struct emit_status **));
static void freeze_thaw_sequence_stack PARAMS ((struct sequence_stack **));
static void freeze_thaw_temp_slot      PARAMS ((struct temp_slot **));
static void freeze_thaw_var_refs_queue PARAMS ((struct var_refs_queue **));
#endif
     ...
     struct function *f;
{
  ...
  if (f->x_parm_reg_stack_loc)
    ...
    PFE_FREE (f->x_parm_reg_stack_loc);
  ...
  /* APPLE LOCAL PFE - expand to pfe_free or free  */
  /* FIXME: Maybe? With non-pfe version now placing f in gc memory
     it is no longer explicitly freed.  So what do we do now?  */
  #if 0
  PFE_FREE (f);
  #endif
}
      ...
      if (GET_MODE (best_p->slot) == BLKmode)
	{
	  ...
	  if (best_p->size - rounded_size >= alignment)
		...
		/* APPLE LOCAL PFE - expand to pfe_ggc_alloc or ggc_alloc  */
	      	  GGC_ALLOC (sizeof (struct temp_slot), PFE_ALLOC_GGC_TEMP_SLOT);
 ...
 restart:
  ...
  if (code == SET)
      ...
      else if (GET_CODE (sub) == REG && GET_MODE (x) != GET_MODE (sub))
	{
	  ...
	  if (size_x <= size_sub
/* APPLE LOCAL Altivec; don't generate subregs of vectors */
     ...
     tree fndecl;
{
  ...
  /* APPLE LOCAL AltiVec */
  /* INIT_CUMULATIVE_ARGS etc moved up from here */
  /* APPLE LOCAL PFE - expand to pfe_calloc or xcalloc  */
  parm_reg_stack_loc = (rtx *) PFE_CALLOC (max_parm_reg, sizeof (rtx),
  					   PFE_ALLOC_RTX_ARRAY);
	  ...
	  if (regno >= max_parm_reg)
	    {
		 ...
		 precisely correct.  */
	      max_parm_reg = regno + 1;
  	      /* APPLE LOCAL PFE - expand to pfe_realloc or xrealloc  */
	      new = (rtx *) PFE_REALLOC (parm_reg_stack_loc,
     ...
     int line;
{
  ...
  current_function_name = PFE_SAVESTRING ((char *)(*decl_printable_name) (subr, 2));
  cfun->decl = subr;
...
/* Freeze/thaw a struct function.  Called for all FUNCTION_DECL nodes
   to freeze/thaw the function pointed to by DECL_SAVED_INSNS(node).  */
void
pfe_freeze_thaw_function (fp)
     struct function **fp;
{
  struct function *save_cfun = cfun, *f = (struct function *)PFE_FREEZE_THAW_PTR (fp);
  int i, j;
  ...
  /* Lines marked with a "+" comment are always preserved.  All others should
  ...
  pfe_freeze_thaw_varasm_status (&f->varasm);
  pfe_freeze_thaw_ptr_fp (&f->name);			/* + */
  PFE_FREEZE_THAW_WALK (f->decl);			/* +? */ /* FIXME: needed? */
  assert (f->outer == NULL);
  PFE_FREEZE_THAW_RTX (f->arg_offset_rtx);
  PFE_FREEZE_THAW_RTX (f->return_rtx);
  PFE_FREEZE_THAW_RTX (f->internal_arg_pointer);
  pfe_freeze_thaw_ptr_fp (&f->cannot_inline);		/* + this is a constant string */
  pfe_freeze_thaw_initial_value_struct (&f->hard_reg_initial_vals);
  PFE_FREEZE_THAW_WALK (f->x_nonlocal_labels);
  PFE_FREEZE_THAW_RTX (f->x_nonlocal_goto_handler_slots);
  PFE_FREEZE_THAW_RTX (f->x_nonlocal_goto_handler_labels);
  PFE_FREEZE_THAW_RTX (f->x_nonlocal_goto_stack_level);
  PFE_FREEZE_THAW_RTX (f->x_cleanup_label);
  PFE_FREEZE_THAW_RTX (f->x_return_label);
  PFE_FREEZE_THAW_RTX (f->x_save_expr_regs);
  PFE_FREEZE_THAW_RTX (f->x_stack_slot_list);
  PFE_FREEZE_THAW_WALK (f->x_rtl_expr_chain);
  PFE_FREEZE_THAW_RTX (f->x_tail_recursion_label);
  PFE_FREEZE_THAW_RTX (f->x_tail_recursion_reentry);
  PFE_FREEZE_THAW_RTX (f->x_arg_pointer_save_area);
  PFE_FREEZE_THAW_RTX (f->x_clobber_return_insn);
  PFE_FREEZE_THAW_WALK (f->x_context_display);
  PFE_FREEZE_THAW_WALK (f->x_trampoline_list);
  PFE_FREEZE_THAW_RTX (f->x_parm_birth_insn);
  PFE_FREEZE_THAW_RTX (f->x_last_parm_insn);

  if (f->x_parm_reg_stack_loc)
    {
      rtx *x_parm_reg_stack_loc = PFE_FREEZE_THAW_PTR (&f->x_parm_reg_stack_loc);

      for (i = f->x_max_parm_reg, j = 0; i > 0; --i, ++j)
	PFE_FREEZE_THAW_RTX (x_parm_reg_stack_loc[j]);
    }
  
  freeze_thaw_temp_slot (&f->x_temp_slots);
  freeze_thaw_var_refs_queue (&f->fixup_var_refs_queue);
  pfe_freeze_thaw_rtvec ((rtvec *)&f->original_arg_vector);
  PFE_FREEZE_THAW_WALK (f->original_decl_initial);
  PFE_FREEZE_THAW_RTX (f->inl_last_parm_insn);
  
#ifdef PFE_FREEZE_THAW_MACHINE_STATUS
  PFE_FREEZE_THAW_MACHINE_STATUS (f);
#endif

  pfe_freeze_thaw_language_function ((struct language_function **)&f->language); /* + */
  PFE_FREEZE_THAW_RTX (f->epilogue_delay_list);
      
  cfun = save_cfun;
}

/* Freeze/thaw the expr_status in struct function.  */
static void
freeze_thaw_expr_status (ep)
     struct expr_status **ep;
{
  struct expr_status *e = PFE_FREEZE_THAW_PTR (ep);
  
  if (!e)
    return;
    
  PFE_FREEZE_THAW_RTX (e->x_saveregs_value);
  PFE_FREEZE_THAW_RTX (e->x_apply_args_value);
  PFE_FREEZE_THAW_RTX (e->x_forced_labels);
  PFE_FREEZE_THAW_RTX (e->x_pending_chain);
}

/* Freeze/thaw the emit_status in struct function.  */
static void
freeze_thaw_emit_status (esp)
     struct emit_status **esp;
{
  struct emit_status *es = PFE_FREEZE_THAW_PTR (esp);
  int i, j;
  rtx *x_regno_reg_rtx;
  tree *regno_decl;
  ...
  freeze_thaw_sequence_stack (&es->sequence_stack);
  pfe_freeze_thaw_ptr_fp (&es->x_last_filename);
  pfe_freeze_thaw_ptr_fp (&es->regno_pointer_align);
  
  regno_decl      = PFE_FREEZE_THAW_PTR (&es->regno_decl);
  x_regno_reg_rtx = PFE_FREEZE_THAW_PTR (&es->x_regno_reg_rtx);
  if (es->regno_pointer_align_length > 0)
    for (i = es->regno_pointer_align_length, j = 0; i > 0; --i, ++j)
      {
	if (regno_decl)
	  PFE_FREEZE_THAW_WALK (regno_decl[j]);
	if (x_regno_reg_rtx)
	  PFE_FREEZE_THAW_RTX (x_regno_reg_rtx[j]);
      }
}

/* Freeze/thaw the sequence_stack in a expr_status.  */
static void
freeze_thaw_sequence_stack (sp)
     struct sequence_stack **sp;
{
  struct sequence_stack *stack = PFE_FREEZE_THAW_PTR (sp);
  ...
  while (stack)
    {
      PFE_FREEZE_THAW_RTX (stack->first);  		/* FIXME: needed? */
      PFE_FREEZE_THAW_RTX (stack->last);  		/* FIXME: needed? */
      PFE_FREEZE_THAW_WALK (stack->sequence_rtl_expr); 	/* FIXME: needed? */
      stack = PFE_FREEZE_THAW_PTR (&stack->next);
    }
}

/* Freeze/thaw the temp_slot in struct function.  */
static void
freeze_thaw_temp_slot (tsp)
     struct temp_slot **tsp;
{
  struct temp_slot *ts = PFE_FREEZE_THAW_PTR (tsp);
  ...
  while (ts)
    {
      PFE_FREEZE_THAW_RTX (ts->slot);
      PFE_FREEZE_THAW_RTX (ts->address);
      PFE_FREEZE_THAW_WALK (ts->type);			/* FIXME: needed? */
      PFE_FREEZE_THAW_WALK (ts->rtl_expr);
      
      ts = PFE_FREEZE_THAW_PTR (&ts->next);
    }
}

/* Freeze/thaw the var_refs_queue in struct function.  */
static void
freeze_thaw_var_refs_queue (qp)
     struct var_refs_queue **qp;
{
  struct var_refs_queue *q = PFE_FREEZE_THAW_PTR (qp);
  ...
  while (q)
    {
      PFE_FREEZE_THAW_RTX (q->modified);
...
DEFINE_CHECK_STRUCT_FUNCTION (emit_status)
DEFINE_CHECK_STRUCT_FUNCTION (sequence_stack)
DEFINE_CHECK_STRUCT_FUNCTION (temp_slot)
DEFINE_CHECK_STRUCT_FUNCTION (var_refs_queue)
