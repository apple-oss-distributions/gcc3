...
struct eh_region;
static struct eh_region *freeze_thaw_eh_region PARAMS ((struct eh_region **));
#endif
...
init_eh_for_function ()
{
  ...
  cfun->eh = (struct eh_status *) PFE_CALLOC (1, sizeof (struct eh_status),
  					      PFE_ALLOC_EH_STATUS);
}
...
expand_eh_region_start ()
{
  ...
  /* APPLE LOCAL PFE - expand to pfe_calloc or xcalloc  */
  new_region = (struct eh_region *) PFE_CALLOC (1, sizeof (*new_region),
  						PFE_ALLOC_EH_REGION);
...
collect_eh_region_array ()
{
  ...
  /* APPLE LOCAL PFE - expand to pfe_calloc or xcalloc  */
  array = (struct eh_region **)PFE_CALLOC (cfun->eh->last_region_number + 1,
  				           sizeof (*array),
  				           PFE_ALLOC_EH_REGION_ARRAY);
...
assign_filter_values ()
{
  ...
  VARRAY_TREE_INIT (cfun->eh->ttype_data, 16, PFE_VARRAY "ttype_data");
  VARRAY_UCHAR_INIT (cfun->eh->ehspec_data, 64, PFE_VARRAY "ehspec_data");
     ...
     struct sjlj_lp_info *lp_info;
{
  ...
  /* APPLE LOCAL PFE - PFE_VARRAY is pfe indicator or null */
  VARRAY_UCHAR_INIT (cfun->eh->action_record_data, 64, PFE_VARRAY "action_record_data");
     ...
     int action;
{
  ...
  if (used >= size)
	...
	PFE_REALLOC (data, sizeof (*data) * size,
		     PFE_ALLOC_CALL_SITE_RECORD);
...
/* Called by pfe_freeze_thaw_function() to freeze/thaw the struct 
   eh_status (private to this file) field of a struct function.  */ 
void 
pfe_freeze_thaw_eh_status (ehp)
  struct eh_status **ehp;
{
  ...
  if ((rp = PFE_FREEZE_THAW_PTR (&eh->region_array)) != NULL)
    {
      for (i = eh->last_region_number; i > 0; --i)
        {
	  r = freeze_thaw_eh_region (&rp[i]);
	  pfe_freeze_thaw_ptr_fp (&r->outer);
	  pfe_freeze_thaw_ptr_fp (&r->inner);
	  pfe_freeze_thaw_ptr_fp (&r->next_peer);
	}
    }
  else if (eh->region_tree)
    {
      /* This code is based on what collect_eh_region_array() does.  */
      rp = &eh->region_tree;
      while (1)
	{
	  r = freeze_thaw_eh_region (rp);
	  if (r->inner)
	    rp = &r->inner;
	  else if (r->next_peer)
	    rp = &r->next_peer;
  ...
  pfe_freeze_thaw_ptr_fp (&eh->try_region);
  PFE_FREEZE_THAW_WALK (eh->protect_list);
  PFE_FREEZE_THAW_RTX (eh->filter);
  PFE_FREEZE_THAW_RTX (eh->exc_ptr);
  pfe_freeze_thaw_varray_tree (&eh->ttype_data);
  pfe_freeze_thaw_ptr_fp (&eh->ehspec_data);
  pfe_freeze_thaw_ptr_fp (&eh->action_record_data);
  if (eh->call_site_data)
    {
  ...
  PFE_FREEZE_THAW_RTX (eh->ehr_handler);
  PFE_FREEZE_THAW_RTX (eh->ehr_label);
  PFE_FREEZE_THAW_RTX (eh->sjlj_fc);
...
freeze_thaw_eh_region (rp)
     struct eh_region **rp;
{
  struct eh_region *r = PFE_FREEZE_THAW_PTR (rp);
  ...
  switch (r->type)
    {
    case ERT_TRY:
      pfe_freeze_thaw_ptr_fp (&r->u.try.catch);
      pfe_freeze_thaw_ptr_fp (&r->u.try.last_catch);
      pfe_freeze_thaw_ptr_fp (&r->u.try.prev_try);
      PFE_FREEZE_THAW_RTX (r->u.try.continue_label);
      break;
    case ERT_CATCH:
      pfe_freeze_thaw_ptr_fp (&r->u.catch.next_catch);
      pfe_freeze_thaw_ptr_fp (&r->u.catch.prev_catch);
      PFE_FREEZE_THAW_WALK (r->u.catch.type_list);
      PFE_FREEZE_THAW_WALK (r->u.catch.filter_list);
      break;
    case ERT_ALLOWED_EXCEPTIONS:
      PFE_FREEZE_THAW_WALK (r->u.allowed.type_list);
      break;
    case ERT_THROW:
      PFE_FREEZE_THAW_WALK (r->u.throw.type);
      break;
    case ERT_CLEANUP:
      PFE_FREEZE_THAW_WALK (r->u.cleanup.exp);
      break;
    case ERT_FIXUP:
      PFE_FREEZE_THAW_WALK (r->u.fixup.cleanup_exp);
      pfe_freeze_thaw_ptr_fp (&r->u.fixup.real_region);
      break;
    case ERT_MUST_NOT_THROW:
    case ERT_UNKNOWN:
      break;
  ...
  PFE_FREEZE_THAW_RTX (r->label);
  PFE_FREEZE_THAW_RTX (r->landing_pad);
  PFE_FREEZE_THAW_RTX (r->post_landing_pad);
  PFE_FREEZE_THAW_RTX (r->resume);
...
DEFINE_CHECK_STRUCT_FUNCTION (eh_status)
DEFINE_CHECK_STRUCT_FUNCTION (eh_region)
